...
bool AravisFrameSource::open(std::string* error) {
    ...
    if (!config_.pixel_format.empty() && config_.pixel_format != "auto") {
        arv_camera_set_pixel_format_from_string(impl_->camera, config_.pixel_format.c_str(), &gerror);
        if (gerror != nullptr) {
            const std::string set_error = to_error_message(gerror);
            gerror = nullptr;
            const char* actual_format = arv_camera_get_pixel_format_as_string(impl_->camera, &gerror);
            std::string actual = actual_format != nullptr ? actual_format : "unknown";
            if (gerror != nullptr) {
                g_clear_error(&gerror);
            }
            if (error) {
                *error = "Failed to set PixelFormat=" + config_.pixel_format +
                         ": " + set_error + "; actual PixelFormat=" + actual +
                         ". Use --pixel-format auto only if grayscale/current camera format is intentional.";
            }
            close();
            return false;
        }
...

    {
        ...
        if (gerror != nullptr) {
            g_clear_error(&gerror);
        } else if (actual_format != nullptr) {
            std::cout << "Aravis PixelFormat: " << actual_format << '\n';
        }
    }
