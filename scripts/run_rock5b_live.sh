...
if [[ "$CVPROJ_SETUP_CAN" == "1" ]]; then
  if ip link show "$CVPROJ_CAN_IFACE" >/dev/null 2>&1; then
    ...
    ip -details link show "$CVPROJ_CAN_IFACE" | sed -n 1,6p
  else
    echo "WARN: CAN interface $CVPROJ_CAN_IFACE not found; continuing without CAN setup" >&2
  fi
fi

echo 123 | sudo -S env LD_LIBRARY_PATH="$LD_LIBRARY_PATH"   ./build-opencv410/cpp/cvproj_capture --config "$CONFIG_PATH" "$@"
