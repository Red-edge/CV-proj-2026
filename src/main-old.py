...
Main entry point with 64-Point Fixed-Grid Optical Flow as ROI Prior.
...
def parse_args():
    p = argparse.ArgumentParser(description="64-Point Motion-Guided YOLO Detection")
...
def main():
    args = parse_args()
    print("="*60)
    print("64-Point Motion-Guided YOLO Detection")
    print("="*60)
    ...
    win = "Motion-Guided YOLO - Q to quit"
    print("\nControls: q=Quit r=Reset s=Save v=Toggle Record +/-=MotionThresh")

    try:
        while True:
            frame = read_frame(use_rs, cam, fb, args.resize_input, args.input_max_width, args.input_max_height)
            if frame is None:
                time.sleep(0.05); continue

            res = pipe.process_frame(frame)
            cv2.imshow(win, res["frame"])
            ...
            if key == ord("q"): break
                ...
                cv2.imwrite(fn, res["frame"]); print(f"💾 {fn}")
            elif key == ord("v"):
                rec.enabled = not rec.enabled; print(f"📹 Record {'ON' if rec.enabled else 'OFF'}")
            ...
            if not skip: rec.write(res["frame"])

    except KeyboardInterrupt: print("\n⏹ Interrupted")
    finally:
        rec.release()
        if use_rs and cam: cam.stop()
        if fb: fb.stop()
